...
def set_nthreads(nthreads):
    ...
    >>> oldn = blosc_cffi.set_nthreads(2)
    >>> blosc_cffi.set_nthreads(1)
    2

    """
    if nthreads > blosc_cffi.MAX_THREADS:
        raise ValueError("the number of threads cannot be larger than %d" %
                         blosc_cffi.MAX_THREADS)
...
def set_blocksize(blocksize):
    ...
    >>> blosc_cffi.set_blocksize(512)
    >>> blosc_cffi.set_blocksize(0)
...
def set_releasegil(gilstate):
    ...
    >>> oldReleaseState = blosc_cffi.set_releasegil(True)
...
def get_cbuffer_sizes(bytesobj):
    ...
    >>> blosc_cffi.get_cbuffer_sizes(c)
    (10, 26, 10)
    """
...
def _check_shuffle(shuffle):
    if shuffle not in [blosc_cffi.NOSHUFFLE, blosc_cffi.SHUFFLE, blosc_cffi.BITSHUFFLE]:
...
def _check_typesize(typesize):
    if not 1 <= typesize <= blosc_cffi.MAX_TYPESIZE:
        raise ValueError("typesize can only be in the 1-%d range." %
                         blosc_cffi.MAX_TYPESIZE)
...
def _check_input_length(input_name, input_len):
    if input_len > blosc_cffi.MAX_BUFFERSIZE:
        raise ValueError("%s cannot be larger than %d bytes" %
                         (input_name, blosc_cffi.MAX_BUFFERSIZE))
...
def compress(bytesobj, typesize=8, clevel=9, shuffle=blosc_cffi.SHUFFLE,
    ...
    >>> c_bytesobj = blosc_cffi.compress(a_bytesobj, typesize=4)
...
def compress_ptr(address, items, typesize=8, clevel=9, shuffle=blosc_cffi.SHUFFLE,
                 cname='blosclz'):
    ...
    >>> c = blosc_cffi.compress_ptr(np_array.__array_interface__['data'][0], \
    ...
    >>> a = Array(*data)
    >>> c = blosc_cffi.compress_ptr(ctypes.addressof(a), items, typesize)
    >>> d = blosc_cffi.decompress(c)
    >>> import struct
...
def decompress(bytesobj, as_bytearray=False):
    ...
    >>> a_bytesobj = a.tostring()
    >>> c_bytesobj = blosc_cffi.compress(a_bytesobj, typesize=4)
    >>> a_bytesobj2 = blosc_cffi.decompress(c_bytesobj)
    >>> a_bytesobj == a_bytesobj2
    True
    >>> b"" == blosc_cffi.decompress(blosc_cffi.compress(b"", 1))
    True
    >>> b"1"*7 == blosc_cffi.decompress(blosc_cffi.compress(b"1"*7, 8))
    True
    >>> type(blosc_cffi.decompress(blosc_cffi.compress(b"1"*7, 8),
...
def decompress_ptr(bytesobj, address):
    ...
    >>> nbytes = blosc_cffi.decompress_ptr(c, np_ans.__array_interface__['data'][0])
    >>> (np_array == np_ans).all()
    True
    ...
    >>> nbytes = blosc_cffi.decompress_ptr(c, ctypes.addressof(out_array))
    >>> import struct
...
def pack_array(array, clevel=9, shuffle=blosc_cffi.SHUFFLE, cname='blosclz'):
    ...
    >>> import numpy
    >>> a = numpy.arange(1e6)
    >>> parray = blosc_cffi.pack_array(a)
...
def unpack_array(packed_array, **kwargs):
    ...
    **kwargs : fix_imports / encoding / errors
    ...
    >>> len(parray) < a.size*a.itemsize
    True
    >>> a2 = blosc_cffi.unpack_array(parray)
    >>> numpy.alltrue(a == a2)
    True
    >>> a = numpy.array(['å', 'ç', 'ø'])
    >>> parray = blosc_cffi.pack_array(a)
...
def print_versions():
    ...
    print("Blosc version: %s" % blosc_cffi.blosclib_version)
    print("Compressors available: %s" % blosc_cffi.cnames)
    print("Compressor library versions:")
    for clib in sorted(blosc_cffi.clib_versions.keys()):
        print("  %s: %s" % (clib, blosc_cffi.clib_versions[clib]))
    ...
    print("Detected cores: %s" % blosc_cffi.ncores)
    print("Number of threads to use by default: %s" % blosc_cffi.nthreads)
    print("-=" * 38)
